...
function PaymentHistoryContent(props) {
  ...
  return (
    <StyledPurchaseHistoryContainer className="w-full">
        ...
        {(rowsPerPage > 0
          ...
          <StyledTableRow key={index}>
            ...
            <StyledTableCell align="left">
              {item.product ?
                <>
                  {t('Purchased')} <span style={{ color: 'blue' }}>{item.amount} {item.product.name}</span>(s) {t('At')} {item.date}
                </>
                :
                <>
                  {item.method == 'paypal'
