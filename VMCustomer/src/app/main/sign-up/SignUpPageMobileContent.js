...
const StyledSignUpButton = styled(motion.button)(({ theme }) => ({
  display: "flex",
  padding: "15px 26px",
...
function SignUpPageMobileContent(props) {
  const { signUp, usergoogleLogin, resendVerificationMail, goToResend, resendEmail } = props;
    ...
    <StyledSignUpContainer>
      ...
      {!goToResend ?
        <div className="sign-form">
          ...
          >
            {t("Already")}&nbsp;&nbsp;
            <span style={{ textDecoration: "underline" }}>{t("Login")}</span>
          </div>
          <StyledSignUpButton
            onClick={() => {
              if (!isSubmit)
          ...
          >
            {!isSubmit ? `${t("Signup")}` : <StyledLoading />}
          </StyledSignUpButton>
        </div>
        :
        <div className="resend-mail-form">
          <p className="resend-mail-title">{t('VerifiTitle')}</p>
          <p className="resend-mail-description">
            {t('VerifiDesc')}
          </p>
          <div className="resend-mail-input">
            {t("Email")}
            <input
              value={resendEmail}
              readOnly
          ...
          <StyledSignUpButton
            ...
            whileTap={!isSubmit && tapAnimation}
          >
            {!isSubmit ? `${t("Resend")}` : <StyledLoading />}
          </StyledSignUpButton>
        </div>
      }
    </StyledSignUpContainer>
...
export default SignUpPageMobileContent;
